...
const currentPath = ref<string[]>([]) // e.g. ['/', 'folder1', 'file.txt']
...
//获取currentPath(一定是folder)对应的那个FileTree对象，如果对应的path是文件的话则应该返回undefined
//除此之外，未初始化时，也会返回undefined
const currentFile = computed<FileTree | undefined>({
  get: () => {
    ...
    for (let i = 0; i < currentPath.value.length; i++) {
      ...
      if (!node) throw new Error(`file底部不应该有文件，出错的path: ${currentPath.value.slice(0, i).join('/')}`)
  ...
  set: (newValue) => {
    ...
    if (targetIndex !== -1) {
      currentLevel[targetIndex] = newValue
    }
  }
})
...
// 处理文件树节点点击
const handleTreeNodeClick = (node: FileTree) => {
